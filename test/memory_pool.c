...
void* Allocate(TYPEINPOOL poolType)
{
    ...
    int blockSize = mpool[poolType].blockSize;
    ...
    for(;;chunk=chunk->next)
    {
        ...
        {
            if(chunk->blocksAvailable == mpool[poolType].blocks)
            {
                if(mpool[poolType].blank_chunk_num > 0)
                {
                    mpool[poolType].blank_chunk_num--;
                }
                else
                {
                    log_err(" mpool[poolType].blank_chunk_num != 0");
                }
            }
...
{
    int chunkLength = mpool[poolType].blocks * mpool[poolType].blockSize;
...
void Deallocate(void* p, TYPEINPOOL poolType)
{
    ...
    *toRelease = deallocChunk->firstAvailableBlock; /* p的下一个可用block为当前的firstAvailableBlock */
    ...
    deallocChunk->firstAvailableBlock = (int)(toRelease - deallocChunk->pData)/blockSize;
